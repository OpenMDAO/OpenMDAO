...
When you have group, or entire models, that are small enough to fit the entire Jacobian in memory you can have OpenMDAO actually assemble the partial derivative Jacobian in memory.
...
The the following example, borrowed from the :ref:`newton solver tutorial<defining_icomps_tutorial>` we assemble the Jacobian at the same level of the model hierarchy as the :ref:`NewtonSolver<nlnewton>` and :ref:`DirectSolver<directsolver>`.
