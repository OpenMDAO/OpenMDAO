...
The following example shows the optimization of a simple beam with rectanglar cross section. The beam has been subdivided into
elements along the length of the beam, and one end is fixed. The goal is to minimize the volume (and hence the mass of the
homogenous beam) by varying the thickness in each element without exceeding a maximum stress constraint while the beam is
...
The problem presented here is also an example of a multipoint implementation, where we create a separate instance of the
