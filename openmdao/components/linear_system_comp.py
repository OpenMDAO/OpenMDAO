""" A component that solves a linear system. """

import numpy as np
from scipy import linalg

from openmdao.core.implicitcomponent import ImplicitComponent


class LinearSystemComp(ImplicitComponent):
    """Solve a linear system, Ax=b.

    Attributes
    ----------
    _lup : object
        lup returned from scipy.linag.lu_factor
    _rhs_cache: dict
        don't know yet
    """

    def __init__(self, **kwargs):
        super(LinearSystemComp, self).__init__(**kwargs)
        # cache
        self._lup = None
        self._rhs_cache = None

    def initialize(self):
        self.metadata.declare('size', value=1, typ=int, desc='the size of the linear system')

    def initialize_variables(self):

        size = self.metadata['size']
        self.add_input("A", val=np.eye(size))
        self.add_input("b", val=np.ones(size))

        self.add_output("x", shape=size)

    def solve_nonlinear(self, inputs, outputs, resids):
        """ Use numpy to solve Ax=b for x.
        """

        # lu factorization for use with solve_linear
        self.lup = linalg.lu_factor(inputs['A'])

        outputs['x'] = linalg.lu_solve(self.lup, inputs['b'])
        resids['x'] = inputs['A'].dot(outputs['x']) - inputs['b']

    def apply_nonlinear(self, inputs, outputs, resids):
        """Evaluating residual for given state."""

        print resids['x'].shape, (inputs['A'].dot(outputs['x'])).shape
        resids['x'] = inputs['A'].dot(outputs['x']) - inputs['b']

    def apply_linear(self, inputs, outputs, d_inputs, d_outputs, d_resids, mode):
        """ Apply the derivative of state variable with respect to
        everything."""

        if mode == 'fwd':

            if 'x' in d_outputs:
                d_resids['x'] += inputs['A'].dot(d_outputs['x'])
            if 'A' in d_inputs:
                d_resids['x'] += d_inputs['A'].dot(outputs['x'])
            if 'b' in d_inputs:
                d_resids['x'] -= d_inputs['b']

        elif mode == 'rev':

            if 'x' in d_outputs:
                d_outputs['x'] += inputs['A'].T.dot(d_resids['x'])
            if 'A' in d_inputs:
                d_inputs['A'] += np.outer(outputs['x'], d_resids['x']).T
            if 'b' in d_inputs:
                d_inputs['b'] -= d_resids['x']

    def solve_linear(self, d_outputs, d_residuals, mode):
        """ LU backsubstitution to solve the derivatives of the linear system."""

        if mode == 'fwd':
            sol_vec, rhs_vec = d_outputs, d_residuals
            t = 0
        else:
            sol_vec, rhs_vec = d_residuals, d_outputs
            t = 1

        if self.rhs_cache is None:
            self.rhs_cache = np.zeros((self.metadata['size'], ))
        rhs = self.rhs_cache

        rhs[:] = rhs_vec['x']

        sol = linalg.lu_solve(self.lup, rhs, trans=t)

        sol_vec['x'] = sol[:]
