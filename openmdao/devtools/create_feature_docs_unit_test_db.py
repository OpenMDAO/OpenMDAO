...
def get_unit_test_source_and_run_outputs(method_path):
    ...
    source_minus_docstrings_with_prints_cleaned = remove_initial_empty_lines_from_source(source_minus_docstrings_with_prints)
    ...
    code_to_run = '\n'.join([lines_before_test_cases,setup_source_code,source_minus_docstrings_with_prints_cleaned,teardown_source_code])
...
conn = sqlite3.connect(sqlite_file)
cur = conn.cursor()
cur.execute("CREATE TABLE {tn}(method_path TEXT, feature TEXT, title TEXT, unit_test_source TEXT, run_outputs TEXT)".format(tn=table_name))
            ...
            for test_method in test_case:
                ...
                if test_doc:
                    ...
                    if test_doc_numpy['Features']:
                        for feature in [ f.strip() for f in test_doc_numpy['Features']]:
                            title = test_doc_numpy['Summary'][0]
                            # from get_test_source_code_for_feature_with_prints import get_unit_test_source_and_run_outputs
                            unit_test_source, run_outputs = get_unit_test_source_and_run_outputs(method_path)
                            cur.execute('insert into {tn} values (?,?,?,?,?)'.format(tn=table_name), 
