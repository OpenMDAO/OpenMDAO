...
class SimpleImplicitComp(ImplicitComponent):
    ...
    def linearize(self, inputs, outputs, resids):
        ...
        # Output equation
        J[('y', 'x')] = numpy.array([1.0])
        J[('y', 'z')] = numpy.array([2.0])

        # State equation
        J[('z', 'z')] = numpy.array([inputs['x'] + 1.0])
        J[('z', 'x')] = numpy.array([outputs['z']])
...
class TestBackTracking(unittest.TestCase):
    ...
    def t0est_newton_with_backtracking(self):
    ...
    def test_bounds_backtracking(self):
        ...
        ls = top.root.nl_solver.set_subsolver('linesearch',
                                              BacktrackingLineSearch(rtol=0.9))
        ls.options['maxiter'] = 100
        ls.options['alpha'] = 10.0
