...
class MultiStatesComp(ImplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('K', shape=(size, size))
        self.add_output('d', shape=(size, num_rhs))
    ...
    def apply_nonlinear(self, inputs, outputs, residuals):
        num_rhs = self.metadata['num_rhs']
        force_vector = np.concatenate([self.metadata['force_vector'], np.zeros(2, num_rhs)])

        for j in range(num_rhs):
            residuals['d'][:, j] = np.dot(inputs['K'], outputs['d']) - force_vector[:, j]
