...
metadata_names = ['name', 'orig', 'alias', 'type', 'size', 'units','global_size', 'lower', 'upper',
                ...
                'linear', 'indices',  'source',  'flat_indices',  'equals', 'parent',  
                    ]

def variable_metadata_to_data_frame(variables_metadata):
    """
    Prep the metadata for putting into a DataFrame that will be displayed by Panel's Tabulator widget.
...
def view_case_recorder(case_recorder_file):
    ...
    # TODO ideally would show more info about all the cases but not sure how to do that concisely yet
    case0 = cases[0]
    ...
    case0_variables_pane = pn.Column(
        f"There are {len(case0.inputs)if case0.inputs else 0} inputs, {len(case0.outputs) if case0.outputs else 0} outputs, and {len(case0.residuals) if case0.residuals else 0} residuals in Case 0",
        pn.Row(
            case0_inputs_pane,
            case0_outputs_pane,
    ...
    metadata_names_in_file = set()  # to collect and dedup all the metadata variables across all the variables in the Problem
    for name, properties in cr.problem_metadata['variables'].items():
        if isinstance(properties, dict):
            ...
            if name not in ['execution_order',]:
                issue_warning(f"problem_metadata['variables'] has unexpected type of '{type(properties)}' for '{name}'", 
    ...
    variables_pane = pn.Column(
        f"Here is the metadata for the {len(cr.problem_metadata['variables'])} variables in the problem",
    ...
    design_vars_pane = pn.Column(
        f"Here is the metadata for the {len(cr.problem_metadata['design_vars'])} design variables in the problem",
    ...
    responses_pane = pn.Column(
        f"Here is the metadata for the {len(cr.problem_metadata['responses'])} responses in the problem",
    ...
    abs2prom_inputs_table_pane = pn.widgets.Tabulator(df_abs2prom_inputs, **tabulator_defaults)
    df_abs2prom_outputs = pd.DataFrame(list(cr.problem_metadata['abs2prom']['output'].items()), columns=["Absolute Name", "Promoted Name"])
    abs2prom_outputs_table_pane = pn.widgets.Tabulator(df_abs2prom_outputs, **tabulator_defaults)
    abs2prom_pane = pn.Column(
        f"There are {len(cr.problem_metadata['abs2prom']['input'])} inputs and {len(cr.problem_metadata['abs2prom']['output'])} outputs in the metadata abs2prom",
    ...
    connections_list_pane = pn.Column(
        f"There are {len(cr.problem_metadata['connections_list'])} items in the metadata connections_list",
    ...
    declare_partials_list_table_pane = pn.widgets.Tabulator(df_declare_partials_list, **tabulator_defaults)

    declare_partials_list_pane = pn.Column(
        f"There are {len(cr.problem_metadata['declare_partials_list'])} items in the metadata declare_partials_list",
