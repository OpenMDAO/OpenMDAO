...
class N2UserInterface {
  ...
  forwardButtonPressed() {
    ...
    let node = this.forwardButtonHistory[this.forwardButtonHistory.length - 1]
      .node;

    if (node.collapsable) {
      this.leftClickedNode = node;
      this.backButtonHistory.push({ node: this.leftClickedNode });
      this.collapse();
    } else {
      ...
      //this.story.push({ node: this.n2Diag.zoomedElement });
      this._setupLeftClick(node);
    }

    this.forwardButtonHistory.pop().node;
    this.n2Diag.update();
  }
  ...
  toggleLegend() {
    ...
    this.n2Diag.dom.parentDiv.querySelector(
      "#showLegendButtonId"
    ).className = this.legend.shown ? "myButton myButtonToggledOn" : "myButton";
  }
  ...
  _setupToolbar() {
    ...
    toolbar.querySelector("#returnToRootButtonId").onclick = function() {
      self.homeButtonClick();
    };
    toolbar.querySelector("#backButtonId").onclick = function() {
      self.backButtonPressed();
    };
    toolbar.querySelector("#forwardButtonId").onclick = function() {
      self.forwardButtonPressed();
    };
    toolbar.querySelector("#upOneLevelButtonId").onclick = function() {
      self.upOneLevelButtonClick();
    };
    toolbar.querySelector("#uncollapseInViewButtonId").onclick = function() {
      self.uncollapseButtonClick(self.n2Diag.zoomedElement);
    };
    toolbar.querySelector("#uncollapseAllButtonId").onclick = function() {
      self.uncollapseButtonClick(self.n2Diag.model.root);
    };
    toolbar.querySelector("#collapseInViewButtonId").onclick = function() {
      self.collapseOutputsButtonClick(self.n2Diag.zoomedElement);
    };
    toolbar.querySelector("#collapseAllButtonId").onclick = function() {
    ...
    toolbar.querySelector("#showLegendButtonId").onclick = function() {
      self.toggleLegend();
    };
    toolbar.querySelector("#toggleSolverNamesButtonId").onclick = function() {
    ...
    for (let i = 8; i <= 14; ++i) {
      let f = (function(idx) {
        return function() {
          self.n2Diag.fontSizeSelectChange(idx);
        };
      })(i);
    ...
    for (let i = 600; i <= 1000; i += 50) {
      let f = (function(idx) {
        return function() {
          self.n2Diag.verticalResize(idx);
        };
      })(i);
    ...
    toolbar.querySelector("#saveSvgButtonId").onclick = function() {
      self.n2Diag.saveSvg();
    };
