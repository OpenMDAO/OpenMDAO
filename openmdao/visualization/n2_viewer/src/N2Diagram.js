...
class N2Diagram {
  constructor(modelJSON) {
    ...
    this.layout = new N2Layout(
      this.model,
      this.zoomedElement,
      this.showLinearSolverNames,
      this.dims
    );
  ...
  saveSvg() {
    ...
    let svgFileName = prompt(
      "Filename to save SVG as",
      "partition_tree_n2.svg"
    );
  ...
  _setupSvgElements() {
    ...
    this.dom.n2OuterGroup = this.dom.svg.append("g").attr("id", "n2outer");

    this.dom.n2InnerGroup = this.dom.n2OuterGroup
      .append("g")
      .attr("id", "n2inner");

    this.dom.pTreeGroup = this.dom.svg
      .append("g")
      .attr("id", "tree")
      .attr("clip-path", "url(#partitionTreeClip");

    this.dom.pSolverTreeGroup = this.dom.svg
      .append("g")
      .attr("id", "solver_tree")
      .attr("clip-path", "url(#solverTreeClip");

    this.dom.n2BackgroundRect = this.dom.n2InnerGroup
      .append("rect")
    ...
    for (let gName of [
      ...
      this.dom.n2Groups[gName] = this.dom.n2InnerGroup
        .append("g")
        .attr("id", "n2" + gName);
    }

    for (let clippedGrp of [
      "elements",
      "gridlines",
      "componentBoxes",
      "dots"
    ...
    for (let oName of ["top", "left", "right", "bottom"]) {
      ogg[oName] = this.dom.n2OuterGroup
        .append("g")
        .attr("id", "n2" + oName)
  ...
  _updateScale() {
    ...
    this.transitCoords.model.x =
      (this.zoomedElement.dims.x
        ? this.layout.size.partitionTree.width -
          this.layout.size.parentNodeWidth
        : this.layout.size.partitionTree.width) /
      (1 - this.zoomedElement.dims.x);
    this.transitCoords.model.y =
      this.layout.size.partitionTree.height / this.zoomedElement.dims.height;

    this.scales.model.x
      .domain([this.zoomedElement.dims.x, 1])
      .range([
        this.zoomedElement.dims.x ? this.layout.size.parentNodeWidth : 0,
        this.layout.size.partitionTree.width
      ]);
    this.scales.model.y
      .domain([
        this.zoomedElement.dims.y,
        this.zoomedElement.dims.y + this.zoomedElement.dims.height
      ])
      .range([0, this.layout.size.partitionTree.height]);

    this.transitCoords.solver.x =
      (this.zoomedElement.solverDims.x
        ? this.layout.size.solverTree.width - this.layout.size.parentNodeWidth
        : this.layout.size.solverTree.width) /
      (1 - this.zoomedElement.solverDims.x);
    this.transitCoords.solver.y =
      this.layout.size.solverTree.height / this.zoomedElement.solverDims.height;

    this.scales.solver.x
      .domain([this.zoomedElement.solverDims.x, 1])
      .range([
        this.zoomedElement.solverDims.x ? this.layout.size.parentNodeWidth : 0,
        this.layout.size.solverTree.width
      ]);
    this.scales.solver.y
      .domain([
        this.zoomedElement.solverDims.y,
    ...
    if (this.scales.firstRun) {
      // first run, duplicate what we just calculated
      ...
      this.dom.n2OuterGroup
        ...
        .attr(
          "transform",
          "translate(" + this.dims.size.partitionTree.width + " 0)"
        );
      ...
      this.dom.n2InnerGroup
        ...
        .attr(
          "transform",
          "translate(" + innerDims.margin + " " + innerDims.margin + ")"
        );
      ...
      this.dom.pSolverTreeGroup
        ...
        .attr(
          "transform",
          "translate(" +
            (this.dims.size.partitionTree.width +
              innerDims.margin +
              innerDims.height +
              innerDims.margin) +
            " " +
      ...
      this.dom.n2Groups.offgrid.bottom
        .attr(
          "transform",
          "translate(0 " + innerDims.height + offgridHeight + ")"
        )
        .attr("width", outerDims.height)
        .attr("height", offgridHeight);
    }
  ...
  _createPartitionCells() {
    ...
    let selection = this.dom.pTreeGroup
    ...
    let nodeEnter = selection
      ...
      .on("mouseover", function(d) {
        if (self.model.abs2prom != undefined) {
          if (d.isParam()) {
            return self.dom.toolTip
              .text(self.model.abs2prom.input[d.absPathName])
              .style("visibility", "visible");
          }
          if (d.isUnknown()) {
            return self.dom.toolTip
              .text(self.model.abs2prom.output[d.absPathName])
              .style("visibility", "visible");
          }
        }
      })
    ...
    nodeEnter
      ...
      .attr("transform", function(d) {
        let anchorX =
          d.prevDims.width * self.prevTransitCoords.model.x -
  ...
  _setupPartitionTransition(d3Refs) {
    ...
    let nodeUpdate = d3Refs.nodeEnter
      .merge(d3Refs.selection)
      .transition(sharedTransition)
      .attr("class", function(d) {
        return "partition_group " + self.style.getNodeClass(d);
      })
    ...
    nodeUpdate
      ...
      .attr("transform", function(d) {
        let anchorX =
          d.dims.width * self.transitCoords.model.x -
          self.layout.size.rightTextMargin;
        return (
          "translate(" +
          anchorX +
          " " +
          (d.dims.height * self.transitCoords.model.y) / 2 +
          ")"
        );
      })
      .style("opacity", function(d) {
  ...
  _createSolverCells() {
    ...
    let selection = self.dom.pSolverTreeGroup
      .selectAll(".solver_group")
    ...
    let nodeEnter = selection
      ...
      .append("svg:g")
      .attr("class", function(d) {
        let solver_class = self.style.getSolverClass(
          self.showLinearSolverNames,
    ...
    nodeEnter
      ...
      .attr("transform", function(d) {
        let anchorX =
          d.prevSolverDims.width * self.prevTransitCoords.solver.x -
          self.layout.size.rightTextMargin;
        return (
          "translate(" +
          anchorX +
          "," +
          (d.prevSolverDims.height * self.prevTransitCoords.solver.y) / 2 +
          ")"
        );
      })
      .style("opacity", function(d) {
  ...
  _setupSolverTransition(d3Refs) {
    ...
    nodeUpdate
      ...
      .attr("transform", function(d) {
        let anchorX =
          d.solverDims.width * self.transitCoords.solver.x -
          self.layout.size.rightTextMargin;
        return (
          "translate(" +
          anchorX +
          "," +
          (d.solverDims.height * self.transitCoords.solver.y) / 2 +
          ")"
        );
      })
      .style("opacity", function(d) {
  ...
  _runSolverTransition(selection) {
    ...
    let nodeExit = selection
      ...
      .attr("transform", function(d) {
        return (
          "translate(" +
          self.scales.solver.x(d.solverDims.x) +
          "," +
          self.scales.solver.y(d.solverDims.y) +
          ")"
        );
  ...
  update(computeNewTreeLayout = true) {
    ...
    if (computeNewTreeLayout) {
      ...
      this.matrix = new N2Matrix(
        ...
        this.layout,
        this.dom.n2Groups,
        this.ui.lastClickWasLeft,
        this.ui.findRootOfChangeFunction,
        this.matrix.nodeSize
      );
    }
  ...
  updateSizes(height, fontSize) {
    ...
    this.dims.size.n2matrix.height = this.dims.size.n2matrix.width = this.dims.size.partitionTree.height = this.dims.size.solverTree.height = height; // Match base height, keep it looking square
  ...
  verticalResize(height) {
    for (let i = 600; i <= 1000; i += 50) {
      ...
      this.dom.parentDiv.querySelector(
        "#idVerticalResize" + i + "px"
      ).innerHTML = newText;
    }
  ...
  mouseOverOnDiagonal(cell) {
    if (this.matrix.cellExists(cell)) this.matrix.mouseOverOnDiagonal(cell);
  }
  ...
  mouseOverOffDiagonal(cell) {
    if (this.matrix.cellExists(cell)) this.matrix.mouseOverOffDiagonal(cell);
  }
