...
class N2Toolbar {
    ...
    async _setupHelp() {
        ...
        html2canvas(document.querySelector("#toolbarLoc"), { logging: false }).then(canvas => {
            self.helpInfo = {
                toolbarImg: canvas.toDataURL(), // Convert the canvas to a PNG
                width: canvas.width,
    ...
    _setupButtonFunctions(n2ui) {
        ...
        this._addButton(new N2ToolbarButtonToggle('#question-button', tooltipBox,
            ...
            e => {
                if (!self.helpInfo) self._setupHelp();
                else new N2Help(self.helpInfo);
            }
        ));
