...
from bokeh.models import Axis, Slider, ColumnDataSource, ContinuousColorMapper
from bokeh.models import ColorBar, FixedTicker, BasicTicker, LinearColorMapper, Range1d
...
class UnstructuredMetaModelVisualization(object):
    ...
    def __init__(self, prob, surrogate_comp, resolution=50):
        ...
        self.output_select.on_change('value', self.output_value_update)
        ...
        self.bot_plot_source = ColumnDataSource(data=dict(bot_slice_x=np.repeat(0, self.resolution), bot_slice_y=np.repeat(0, self.resolution)))
        self.right_plot_source = ColumnDataSource(data=dict(left_slice_x=np.repeat(0, self.resolution), left_slice_y=np.repeat(0, self.resolution)))
        self.source = ColumnDataSource(data=dict(x=np.repeat(0, self.resolution), y=np.repeat(0, self.resolution)))
        ...
        self.sliders_and_selects = row(
            ...
            self.y_input, self.output_select, self.scatter_distance)
        )
    ...
    def make_predictions(self, data):
        ...
        inputs = np.empty([self.resolution**2, self.nx])
    ...
    def contour_data(self):
        ...
        # Append the key (input_list) and the values copied from xe to pred_dict 
        ...
        # Contour Plot
        self.contour_plot = figure(tooltips=[(self.x_input.value, "$x"), (self.y_input.value, "$y"), 
    ...
    def right_plot(self):
        ...
        s1 = figure(plot_width=200, plot_height=500, x_range=(min(x), max(x)), y_range=(min(y_data),max(y_data)), 
        title="{} vs {}".format(self.y_input.value, self.output_select.value), tools="")
        s1.xaxis.axis_label = self.output_select.value
        ...
        self.right_plot_source.data = dict(left_slice_x=np.repeat(x_value, self.resolution), left_slice_y=y_data, 
        x1=np.array([x+self.dist_range for x in np.repeat(x_value, self.resolution)]), x2=np.array([x-self.dist_range for x in np.repeat(x_value, self.resolution)]))
    ...
    def bot_plot(self):
        ...
        s2.yaxis.axis_label = self.output_select.value
        s2.line(x, y)
        ...
        self.bot_plot_source.data = dict(bot_slice_x=self.x_data, bot_slice_y=np.repeat(self.y_value, self.resolution))
    ...
    def training_points(self):
        ...
        """
        Calculates the training points and returns and array containing the position and alpha opacity of the training points nearest to the surrogate line.
