...
class TestOptimizationReport(unittest.TestCase):
    ...
    def test_opt_report_scalar_desvars(self):
        # Same test as test_opt_report_array_desvars but includes commented out code
    ...
    def test_opt_report_scalar_constraint(self):
        ...
        # model.add_constraint('con1', lower=11.5, upper=12.0)  # lower and upper - much less than lower
        # model.add_constraint('con1', lower=-1, upper=0.5)  # lower and upper - greater than upper
        # model.add_constraint('con1', lower=-6, upper=-5.5)  # lower and upper - much greater than upper
    ...
    def test_opt_report_equality_constraint(self):
        prob = om.Problem()
        model = prob.model
    ...
    def test_opt_report_array_constraint(self):
        ...
        # prob['x'] = np.arange(size) - 100.0 + 1000.
        prob['x'] = np.arange(size) - 100.0

        prob.final_setup()

        # Not running since we want to show when variables are outside of constraints
    ...
    def test_opt_report_equality_constraint(self):
        ...
        model.add_constraint('areas', equals=np.array([24.0, 21.0, 3.5, 17.5])) # no equality constraints met
        # use these when testing to see how visual looks for different situations
        # model.add_constraint('areas', equals=np.array([1.0, 21.0, 1.0, 17.5]))  # Some equality constraints met
        # model.add_constraint('areas', equals=np.array([1.0, 1.0, 1.0, 1.0]))  # All equality constraints met

        prob.setup()

        failed = prob.final_setup()
        opt_report(prob)
    ...
    # def test_simple_array_comp2D_eq_con_no_driver_run(self):  # So we can see if the equality constraint not met
    #
    #     prob = om.Problem()
    #     model = prob.model
    #
    ...
    #
    #
    # def test_opt_report_pyoptsparse_compare_outputs(self):  # has upper and lower arrays for the desvars
    #     import numpy as np
    #
    #     import openmdao.api as om
    #     from openmdao.test_suite.components.sellar import SellarDerivativesGrouped
    ...
    #     opt_report(prob)
    #
    # def test_opt_report_pyoptsparse_compare_outputs_upper_only(self):  # has upper array for the desvars
    #     import numpy as np
    #
    #     import openmdao.api as om
    #     from openmdao.test_suite.components.sellar import SellarDerivativesGrouped
    ...
    #     prob.driver.opt_settings['ACC'] = 1e-13
    #
    #     model.add_design_var('z', upper=np.array([10.0, 10.0]))
    #     model.add_design_var('x', lower=0.0, upper=10.0)
    #     model.add_objective('obj')
    #     model.add_constraint('con1', upper=0.0)
    #     model.add_constraint('con2', upper=0.0)
    ...
    #     opt_report(prob)
    #
    # def test_opt_report_pyoptsparse_compare_outputs_lower_only(self):  # has only lower array for the desvars
    #     import numpy as np
    #
    #     import openmdao.api as om
    #     from openmdao.test_suite.components.sellar import SellarDerivativesGrouped
    ...
    #     opt_report(prob)
    #
    # def test_opt_report_array_vars(self):   # with lower array constraint
    #     size = 200  # how many items in the array
    #
    #     class Adder(om.ExplicitComponent):
    #         """
    ...
    #     prob.model.add_constraint('x', lower=np.array(cons) / 100)
    #
    #     prob.driver = om.ScipyOptimizeDriver()
    #
    #     prob.setup()
    ...
    #     opt_report(prob)
    #
    # def test_opt_report_array_vars_upper_constraint(self):   # with upper array constraint
    #     size = 200  # how many items in the array
    #
    #     class Adder(om.ExplicitComponent):
    #         """
    ...
    #     opt_report(prob)
    #
    # def test_opt_report_array_vars_upper_and_lower_constraint(self):   # with upper and lower array constraint
    #     size = 200  # how many items in the array
    #
    #     class Adder(om.ExplicitComponent):
    #         """
    ...
    #
    #     # prob.model.add_constraint('x', upper=-15.0)
    #     prob.model.add_constraint('x', lower=np.array(cons) / 100, upper=30 + np.array(cons) / 50)
    #     # prob.model.add_constraint('x', lower=np.array(cons) / 100)
    #
    #     prob.driver = om.ScipyOptimizeDriver()
    #
