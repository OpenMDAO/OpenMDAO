...
def _get_multid_circle_problem(flat_indices=False):
    ...
    if flat_indices:
        # Need to convert the multi-indices in `EVEN_IND` into flat indices.
        EVEN_IND_FLAT = np.ravel_multi_index(EVEN_IND, SHAPE).flatten()
        prob.model.add_constraint('theta_con.g', lower=-1e-5, upper=1e-5, indices=EVEN_IND_FLAT, ref=2.2, flat_indices=True)
    else:
