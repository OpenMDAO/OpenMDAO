...
class TestDriver(unittest.TestCase):
    ...
    def test_vector_bounds_inf(self):
        ...
        model.add_design_var('px.x', ref=np.array([.1, .01]))
        model.add_constraint('comp.y2', ref=np.array([.2, .005]))

        prob.setup()
        
        desvars = model.get_design_vars()
        
        np.testing.assert_array_equal(desvars['px.x']['upper'], sys.float_info.max)
        np.testing.assert_array_equal(desvars['px.x']['lower'], -sys.float_info.max)
