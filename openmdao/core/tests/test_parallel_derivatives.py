...
class CheckParallelDerivColoringEfficiency(unittest.TestCase):
    ...
    def test_parallel_deriv_coloring_overlap_err(self):
        ...
        with self.assertRaises(Exception) as ctx:
            prob.setup(mode='rev')
        self.assertEqual(str(ctx.exception), "\nConnection errors for problem 'parallel_deriv_coloring_overlap_err':\n   <model> <class Group>: response 'pg.dc2.y' has overlapping dependencies on the same rank with other responses in parallel_deriv_color 'a'.")
