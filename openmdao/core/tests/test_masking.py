...
class CSCMaskingTestCase(unittest.TestCase):
    def test_csc_masking_fwd(self):
        ...
        range_obj, range_jac = _range_case('fwd')
        obj, jac = _masking_case('fwd')

        assert_almost_equal(base_objective, obj, decimal=7)
        assert_almost_equal(base_objective, range_obj, decimal=7)

        for key in jac:
            assert_almost_equal(base_jac[key], jac[key], decimal=7)
            assert_almost_equal(base_jac[key], range_jac[key], decimal=7)
