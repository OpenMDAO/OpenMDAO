...
class TestProblem(unittest.TestCase):
    ...
    def test_feature_configure_add_input_output(self):
        ...
        class ExternalDataComp(om.ExplicitComponent):
            """
            Simulate data generated by an external source/code
            """
            def setup(self):
                n = 3  # number of points may not be known a priori
        ...
        class ForceModel(om.Group):
            def setup(self):
                self.add_subsystem('external_data', ExternalDataComp(),
            ...
            def configure(self):
                ...
                # after the external_data component has been setup.

                ## list_outputs currently does not work until model has been run
                # external_outputs = self.external_data.list_outputs()

                external_outputs = self.external_data._var_rel2meta
                data_shape = external_outputs['thrust']['shape']
