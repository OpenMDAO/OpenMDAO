...
class TestGetSetVariables(unittest.TestCase):
    ...
    def test_serial_multi_src_inds_units_promoted_no_src(self):
        ...
        self.assertEqual(str(cm.exception), "Group (<model>): The following inputs, ['C1.x', 'C2.x', 'C3.x'], promoted to 'x', are connected but their metadata entries ['units'] differ. Call <group>.set_input_defaults('x', units=?), where <group> is the Group named '' to remove the ambiguity.")
