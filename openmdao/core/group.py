...
from openmdao.utils.array_utils import array_connection_compatible, _flatten_src_indices, shape_to_len
...
from openmdao.utils.om_warnings import issue_warning, UnitsWarning, UnusedOptionWarning, PromotionWarning, MPIWarning
...
class _PromotesInfo(object):
    ...
    def __repr__(self):  # pragma no cover
        return (
            f"_PromotesInfo(src_indices={self.src_indices}, flat={self.flat}, "
    ...
    def copy(self):
        return _PromotesInfo(self.src_indices.copy(), self.flat, self.src_shape, self.promoted_from, self.prom)
    ...
    def compare(self, other):
        ...
        if isinstance(self_srcinds, np.ndarray) and isinstance(other_srcinds, np.ndarray):
            if self_srcinds.shape != other_srcinds.shape or not np.all(self_srcinds == other_srcinds):
    ...
    **kwargs : dict
    ...
    def set_input_defaults(self, name, val=_UNDEFINED, units=None, src_shape=None):
        ...
        if val is _UNDEFINED:
            ...
            if src_shape is not None:
                issue_warning(
                    "value was set in set_input_defaults, so ignoring " f"value {src_shape} of src_shape.",
                    prefix=self.msginfo,
        ...
        if name in dct:
            ...
            if overlap:
                issue_warning(
                    f"Setting input defaults for input '{name}' which "
    ...
    def _get_matvec_scope(self, excl_sub=None):
        ...
        if excl_sub is None:
            ...
            scope_in = frozenset(self._conn_global_abs_in2out).intersection(self._var_allprocs_abs2meta["input"])
                ...
                if src_indices is not None:
                    if not (np.ndim(ref) == 0 and np.ndim(ref0) == 0):
                        ...
                        if meta_out["distributed"]:
                            raise RuntimeError(
                                "{}: vector scalers with distrib vars " "not supported yet.".format(self.msginfo)
                            )

                        if not src_indices._flat_src:
                            src_indices = _flatten_src_indices(
                                src_indices, meta_in["shape"], meta_out["global_shape"], meta_out["global_size"]
                            )
    ...
    def _setup_procs(self, pathname, comm, mode, prob_meta):
        ...
        if self._num_par_fd > 1:
            ...
            if comm.size > 1:
                ...
                if self._owns_approx_jac or info["dynamic"] or info["static"] is not None:
                    ...
                    msg = "%s: num_par_fd = %d but FD is not active." % (self.msginfo, self._num_par_fd)
                    raise RuntimeError(msg)
            elif not MPI:
                msg = (
                    f"MPI is not active but num_par_fd = {self._num_par_fd}. No parallel "
        ...
        if MPI:
            ...
            try:
                sub_inds, sub_comm, sub_proc_range = self._mpi_proc_allocator(proc_info, len(allsubs), comm)
            except ProcAllocationError as err:
                if err.sub_inds is None:
                    raise RuntimeError("%s: %s" % (self.msginfo, err.msg))
                else:
                    raise RuntimeError(
                        "%s: MPI process allocation failed: %s for the following "
                        "subsystems: %s" % (self.msginfo, err.msg, [allsubs[i].system.name for i in err.sub_inds])
                    )

            self._subsystems_myproc = [allsubs[ind].system for ind in sub_inds]

            # Define local subsystems
            if self._mpi_proc_allocator.parallel and not (np.sum([minp for minp, _, _, _ in proc_info]) <= comm.size):
            ...
            for s in self.system_iter(recurse=True, include_self=True):
                ...
                for typ in iotypes:
                    ...
                    for prom, alist in s._var_allprocs_prom2abs_list[typ].items():
                        ...
                        t_remprom2abs[prefix + prom].update(
                            n for n in abs_names if n in vars_to_gather and vars_to_gather[n] == myrank
                        )
        ...
        for absname in chain(abs2meta["input"], abs2meta["output"]):
            if absname in prom2abs_in:
                for name in prom2abs_in[absname]:
                    if name != absname:
                        raise RuntimeError(
                            f"{self.msginfo}: Absolute variable name '{absname}'"
                ...
                if absname != prom2abs_out[absname][0]:
                    raise RuntimeError(
                        f"{self.msginfo}: Absolute variable name '{absname}' is"
    ...
    def _top_level_post_sizes(self):
        ...
        if self.comm.size > 1:
            ...
            dist_ins = (
                n
                for n, m in all_abs2meta_in.items()
                if m["distributed"] or (conns[n].startswith("_auto_ivc.") and all_abs2meta_out[conns[n]]["distributed"])
            )
                ...
                for comp in self.system_iter(recurse=True, typ=Component):
                    if comp.pathname in dcomp_names:
                        added_src_inds.extend(
                            comp._update_dist_src_indices(conns, all_abs2meta, abs2idx, self._var_sizes)
                        )
                ...
                if out_dist and not in_dist:
                    ...
                    if abs_in in abs2meta_in:  # input is local
                        ...
                        if src_inds is not None:
                            ...
                            if shaped is None:
                                self._collect_error(
                                    f"For connection from '{abs_out}' to '{abs_in}'"
                    ...
                    if err == 1:
                        self._collect_error(
                            f"{self.msginfo}: Can't connect distributed output "
    ...
    def _resolve_src_indices(self):
        ...
        for tgt, plist in self._problem_meta["abs_in2prom_info"].items():
            ...
            current_pinfo = _PromotesInfo(src_shape=root_shape, prom=self._var_allprocs_abs2prom["input"][tgt])
            ...
            for i, pinfo in enumerate(plist):
                if pinfo is None:
                    ...
                    try:
                        ...
                        self._collect_error(
                            f"When connecting '{src}' to " f"'{pinfo.prom_path()}': {exc}",
                            exc_type=type_exc,
                            tback=tb,
                            ident=(src, tgt),
                        )
                    ...
                    try:
                        ...
                        sinds = convert_src_inds(
                            current_pinfo.src_indices, current_pinfo.src_shape, pinfo.src_indices, pinfo.src_shape
                        )
                    except Exception:
                        type_exc, exc, tb = sys.exc_info()
                        self._collect_error(
                            f"When connecting '{conns[tgt]}' to "
                    ...
                    current_pinfo = _PromotesInfo(
                        src_indices=src_indices,
                        src_shape=root_shape,
                        flat=True,
                        promoted_from=pinfo.promoted_from,
                        prom=pinfo.prom,
                    )
    ...
    def _setup_var_data(self):
        ...
        for subsys in self._subsystems_myproc:
            ...
            for io in ["input", "output"]:
                ...
                var_discrete[io].update({sub_prefix + k: v for k, v in subsys._var_discrete[io].items()})
            ...
            if isinstance(subsys, Group):
                ...
                for sub_prom, metalist in subsys._group_inputs.items():
                    ...
                    if key not in self._group_inputs:
                        self._group_inputs[key] = [{"path": self.pathname, "prom": key, "auto": True}]
        ...
        if self.comm.size > 1 and self._mpi_proc_allocator.parallel:
            ...
            if mysub and mysub.comm.rank == 0 and (mysub._full_comm is None or mysub._full_comm.rank == 0):
                raw = (
                    allprocs_discrete,
                    allprocs_prom2abs_list,
                    allprocs_abs2meta,
                    self._has_output_scaling,
                    self._has_output_adder,
                    self._has_resid_scaling,
                    self._group_inputs,
                    self._has_distrib_vars,
                )
            else:
                raw = (
                    {"input": {}, "output": {}},
        ...
        for prom_name, abs_list in allprocs_prom2abs_list["output"].items():
            if len(abs_list) > 1:
                self._collect_error(
                    "{}: Output name '{}' refers to "
                    "multiple outputs: {}.".format(self.msginfo, prom_name, sorted(abs_list))
                )
        ...
        if self._group_inputs:
            ...
            if extra:
                ...
                if ex:
                    self._collect_error(
                        f"{self.msginfo}: The following group inputs, passed to "
                    ...
                    if key in submeta:
                        if fullmeta[key] is _UNDEFINED:
                            ...
                            if origin != top_origin:
                                msg = (
                                    f"Group '{top_origin}' did not set a default "
                            ...
                            if not eq:
                                ...
                                if not origin or submeta["path"].startswith(origin + "."):
                                    msg = (
                                        f"Groups '{origin}' and '{submeta['path']}' "
                                    ...
                                    self._collect_error(
                                        f"{self.msginfo}: The subsystems {origin} "
                            ...
                            if pinfo is not None:
                                ...
                                if p2 is not None:
                                    ...
                                    if p2.src_shape is not None and p2.src_shape != src_shape:
                                        self._collect_error(
                                            f"{self.msginfo}: src_shape {src_shape}"
                                ...
                                abs_in2prom_info[tgt][tree_level] = _PromotesInfo(
                                    src_shape=src_shape, prom=prom, promoted_from=self.pathname
                                )
    ...
    def _setup_var_sizes(self):
        ...
        if self.comm.size > 1:
            if (
                self._has_distrib_vars
                or self._contains_parallel_group
                or not np.all(self._var_sizes["output"])
                or not np.all(self._var_sizes["input"])
            ):
                if self._distributed_vector_class is not None:
                    self._vector_class = self._distributed_vector_class
                else:
                    raise RuntimeError(
                        "{}: Distributed vectors are required but no distributed "
    ...
    def _setup_global_connections(self, conns=None):
        ...
        for prom_in, (prom_out, src_indices, flat) in self._manual_connections.items():
            ...
            if not (prom_out in allprocs_prom2abs_list_out or prom_out in allprocs_discrete_out):
                if prom_out in allprocs_prom2abs_list_in or prom_out in allprocs_discrete_in:
                    msg = (
                        f"{self.msginfo}: Attempted to connect from '{prom_out}' to "
                        + f"'{prom_in}', but '{prom_out}' is an input. "
                        + "All connections must be from an output to an input."
                    )
                else:
                    guesses = get_close_matches(
                        prom_out, list(allprocs_prom2abs_list_out.keys()) + list(allprocs_discrete_out.keys())
                    )
                    msg = (
                        f"{self.msginfo}: Attempted to connect from '{prom_out}' to "
            ...
            if not (prom_in in allprocs_prom2abs_list_in or prom_in in allprocs_discrete_in):
                if prom_in in allprocs_prom2abs_list_out or prom_in in allprocs_discrete_out:
                    ...
                    guesses = get_close_matches(
                        prom_in, list(allprocs_prom2abs_list_in.keys()) + list(allprocs_discrete_in.keys())
                    )
                    msg = (
                        f"{self.msginfo}: Attempted to connect from '{prom_out}' to "
                ...
                if src_indices is not None:
                    ...
                    if abs_in in abs2meta:
                        ...
                        if abs_in2prom_info[abs_in][0] is None:
                            try:
                                abs_in2prom_info[abs_in][0] = _PromotesInfo(
                                    src_indices=src_indices, flat=flat, prom=abs_in
                                )
                            except Exception:
                                type_exc, exc, tb = sys.exc_info()
                                self._collect_error(
        ...
        if len(conn_list) > len(global_abs_in2out):
            ...
            if dup_info:
                ...
                self._collect_error(
                    f"{self.msginfo}: The following inputs have multiple " f"connections: {', '.join(dup)}."
                )
    ...
    def _setup_dynamic_shapes(self):
            ...
            for name, meta in self._var_allprocs_abs2meta[io].items():
                if meta["shape_by_conn"]:
                    if name in conn:  # it's a connected input
                        ...
                        if name in rev_conn:  # connected output
                            ...
                            if fail:
                                self._collect_error(
                                    f"{self.msginfo}: 'shape_by_conn' was set for " f"unconnected variable '{name}'."
                                )
                ...
                if meta["copy_shape"]:
                    ...
                    if abs_from in all_abs2prom_in or abs_from in all_abs2prom_out:
                        ...
                        self._collect_error(
                            f"{self.msginfo}: Can't copy shape of variable " f"'{abs_from}'. Variable doesn't exist."
                        )
                ...
                for node in graph.neighbors(known):
                    if node in seen:
                        ...
                        if shape != known_shape and not (dist ^ known_dist):
                            self._collect_error(
                                f"{self.msginfo}: Shape mismatch, {shape} vs. "
        ...
        if unresolved:
            ...
            self._collect_error(
                f"{self.msginfo}: Failed to resolve shapes for {unresolved}. "
    ...
    def _setup_connections(self):
        ...
        for abs_in, abs_out in global_abs_in2out.items():
            ...
            if out_subsys != in_subsys:
                ...
                if nproc > 1 and self._vector_class is None:
                    ...
                    if (
                        abs_in not in abs2meta_in
                        or abs_out not in abs2meta_out
                        or abs2meta_in[abs_in]["distributed"]
                        or abs2meta_out[abs_out]["distributed"]
                    ):
                        self._vector_class = self._distributed_vector_class

            # if connected output has scaling then we need input scaling
            if not self._has_input_scaling and not (abs_in in allprocs_discrete_in or abs_out in allprocs_discrete_out):
                ...
                self._has_input_scaling = (
                    self._has_output_scaling
                    or self._has_resid_scaling
                    or (in_units and out_units and in_units != out_units)
                )
        ...
        for abs_in, abs_out in self._conn_discrete_in2out.items():
            ...
            try:
                ...
                self._collect_error(
                    f"{self.msginfo}: Can't connect continuous output '{abs_out}' " f"to discrete input '{abs_in}'.",
                    ident=(abs_out, abs_in),
                )
                continue
        ...
        for abs_in, abs_out in abs_in2out.items():
            ...
            if out_units:
                if not in_units:
                    if not _is_unitless(out_units):
                        msg = (
                            f"Output '{abs_out}' with units of '{out_units}' "
                ...
                if not _is_unitless(in_units):
                    msg = (
                        f"Input '{abs_in}' with units of '{in_units}' is "
            ...
            if abs_in in abs2meta_in:
                ...
                if src_indices is None and out_shape != in_shape:
                    ...
                    if in_shape is None or out_shape is None or not array_connection_compatible(in_shape, out_shape):
                        self._collect_error(
                            ...
                            f"but the target shape is {in_shape}.",
                            ident=(abs_out, abs_in),
                        )
                        continue
                    ...
                    try:
                        shp = out_shape if all_meta_out["distributed"] else all_meta_out["global_shape"]
                    ...
                    if not src_indices._flat_src and (len(src_indices.indexed_src_shape) > len(out_shape)):
                        self._collect_error(
    ...
    @collect_errors
    def promotes(
        self, subsys_name, any=None, inputs=None, outputs=None, src_indices=None, flat_src_indices=None, src_shape=None
    ):
        """
        ...
        if isinstance(any, str):
            self._collect_error(
                f"{self.msginfo}: Trying to promote any='{any}', "
                "but an iterator of strings and/or tuples is required."
            )
            return
        if isinstance(inputs, str):
            self._collect_error(
                f"{self.msginfo}: Trying to promote inputs='{inputs}', "
                "but an iterator of strings and/or tuples is required."
            )
            return
        if isinstance(outputs, str):
            self._collect_error(
                f"{self.msginfo}: Trying to promote outputs='{outputs}', "
                "but an iterator of strings and/or tuples is required."
            )
            return
        ...
        if src_indices is None:
            ...
            if flat_src_indices is not None or src_shape is not None:
                issue_warning(
                    f"ignored flat_src_indices and/or src_shape because" " src_indices was not specified.",
                    prefix=self.msginfo,
            ...
            try:
                ...
                type_exc, exc, tb = sys.exc_info()
                self._collect_error(
                    f"{self.msginfo}: When promoting {promoted} from " f"'{subsys_name}': {exc}",
                    exc_type=type_exc,
                    tback=tb,
            ...
            if outputs:
                self._collect_error(
                    f"{self.msginfo}: Trying to promote outputs {outputs} while "
            ...
            try:
                ...
                if inputs is not None:
                    lst.extend(inputs)
                self._collect_error(
                    f"{self.msginfo}: When promoting {sorted(lst)}: {err}", ident=(self.pathname, tuple(lst))
                )
                return
        ...
        list_comp = [i if isinstance(i, tuple) else (i, i) for i, _ in subsys._var_promotes["input"]]

        for original, new in list_comp:
            for original_inside, new_inside in list_comp:
                if original == original_inside and new != new_inside:
                    self._collect_error(
                        "%s: Trying to promote '%s' when it has been aliased to "
        ...
        if self._problem_meta is not None:
            if self._problem_meta["config_info"] is not None:
                self._problem_meta["config_info"]._prom_added(self.pathname, any=any, inputs=inputs, outputs=outputs)
    ...
    def add_subsystem(
        ...
        if self._setup_procs_finished:
            raise RuntimeError(f"{self.msginfo}: Cannot call add_subsystem in " "the configure method.")

        if inspect.isclass(subsys):
            raise TypeError(
                f"{self.msginfo}: Subsystem '{name}' should be an instance, but a "
        ...
        if hasattr(self, name) and not isinstance(getattr(self, name), System):
            ...
            raise RuntimeError(
                f"{self.msginfo}: Can't add subsystem '{name}' because an attribute " f"with that name already exits."
            )

        if not isinstance(subsys, System):
            raise TypeError(
                f"{self.msginfo}: Subsystem '{name}' should be a System instance, but "
                f"an instance of type {type(subsys).__name__} was found."
            )

        if proc_group is not None and not isinstance(proc_group, str):
            raise TypeError(
                f"{self.msginfo}: proc_group must be a str or None, but is of type " f"'{type(proc_group).__name__}'."
            )
        ...
        if isinstance(promotes, str) or isinstance(promotes_inputs, str) or isinstance(promotes_outputs, str):
            raise RuntimeError(f"{self.msginfo}: promotes must be an iterator of strings and/or " "tuples.")
        ...
        if promotes:
            subsys._var_promotes["any"] = [(p, prominfo) for p in sorted(promotes, key=lambda x: "*" not in x)]
        if promotes_inputs:
            subsys._var_promotes["input"] = [(p, prominfo) for p in sorted(promotes_inputs, key=lambda x: "*" not in x)]
        if promotes_outputs:
            subsys._var_promotes["output"] = [
                (p, prominfo) for p in sorted(promotes_outputs, key=lambda x: "*" not in x)
            ]
        ...
        if not isinstance(min_procs, int) or min_procs < 1:
            raise TypeError(f"{self.msginfo}: min_procs must be an int > 0 but ({min_procs}) was " "given.")
        if max_procs is not None and (not isinstance(max_procs, int) or max_procs < min_procs):
            raise TypeError(
                f"{self.msginfo}: max_procs must be None or an int >= min_procs but " f"({max_procs}) was given."
            )
        if isinstance(proc_weight, Number) and proc_weight < 0:
            raise TypeError(f"{self.msginfo}: proc_weight must be a float > 0. but ({proc_weight}) " "was given.")
    ...
    def connect(self, src_name, tgt_name, src_indices=None, flat_src_indices=None):
        ...
        if isinstance(src_indices, str):
            ...
            self._collect_error(
                f"{self.msginfo}: src_indices must be a slice, int, or index array."
        ...
        if src_indices is not None:
            try:
                ...
                type_exc, exc, tb = sys.exc_info()
                self._collect_error(
                    f"{self.msginfo}: When connecting from '{src_name}' to " f"'{tgt_name}': {exc}",
                    exc_type=type_exc,
                    tback=tb,
                )
                return
        ...
        for manual_connections in [self._manual_connections, self._static_manual_connections]:
            if tgt_name in manual_connections:
                ...
                self._collect_error(f"{self.msginfo}: Input '{tgt_name}' is already connected to " f"'{srcname}'.")
                return

        # source and target should not be in the same system
        if src_name.rsplit(".", 1)[0] == tgt_name.rsplit(".", 1)[0]:
            self._collect_error(
                f"{self.msginfo}: Output and input are in the same System for "
    ...
    def set_order(self, new_order):
        ...
        """
        if self._problem_meta is not None and self._problem_meta["setup_status"] == _SetupStatus.POST_CONFIGURE:
        ...
        if oldset != newset:
            ...
            if missing:
                msg.append("%s: %s expected in subsystem order and not found." % (self.msginfo, sorted(missing)))

            extra = newset - oldset
            if extra:
                msg.append(
                    "%s: subsystem(s) %s found in subsystem order but don't exist." % (self.msginfo, sorted(extra))
                )
        ...
        if len(newset) < len(new_order):
            ...
            raise ValueError("%s: Duplicate name(s) found in subsystem order list: %s" % (self.msginfo, sorted(dupes)))
        ...
        try:
            if self._discrete_inputs or self._discrete_outputs:
                self.guess_nonlinear(
                    self._inputs, self._outputs, self._residuals, self._discrete_inputs, self._discrete_outputs
                )
            else:
    ...
    def _iter_call_apply_linear(self):
        ...
        """
        return (
            (self._owns_approx_jac and self._jacobian is not None)
            or self._assembled_jac is not None
            or not self._linear_solver.does_recursive_applies()
        )
        ...
        if self._owns_approx_jac:
            ...
            for subsys in self._subsystems_myproc:
                if subsys.pathname in rel_systems:
                    do_ln = sub_do_ln and (
                        subsys._linear_solver is not None and subsys._linear_solver._linearize_children()
                    )
        ...
        for name, attr in (("step", step), ("form", form), ("step_calc", step_calc)):
            if attr is not None:
                if name in default_opts:
                    ...
                    raise RuntimeError("%s: '%s' is not a valid option for '%s'" % (self.msginfo, name, method))
    ...
    def _setup_partials(self):
        ...
        if self._has_distrib_vars and self._owns_approx_jac:
            ...
            for iname, meta in self._var_allprocs_abs2meta["input"].items():
                if meta["has_src_indices"] and meta["distributed"] and iname not in self._conn_abs_in2out:
    ...
    def _update_wrt_matches(self, info):
        ...
        if info.get("dynamic") and info["coloring"] is None and self._owns_approx_of:
            if not wrt_colors_matched:
                raise ValueError(
                    "{}: Invalid 'wrt' variable(s) specified for colored approx "
    ...
    def _setup_approx_partials(self):
        ...
        if info["coloring"] is not None and (self._owns_approx_of is None or self._owns_approx_wrt is None):
            method = info["method"]
        else:
    ...
    def _get_auto_ivc_out_val(self, tgts, vars_to_gather, abs2meta_in):
                        ...
                        if shape is None:
                            ...
                            if inds._src_shape is None:
                                try:
                                    ...
                                    self._collect_error(
                                        f"When promoting '{pinfo.prom}' from system"
            ...
            if src_indices is not None:
                if val is None:
                    ...
                    try:
                        ...
                        msg = (
                            f"{self.msginfo}: The source indices "
                ...
                if val is None:
                    ...
                    if val.size > 1:
                        src = self._conn_global_abs_in2out[tgt]
                        self._collect_error(
                            f"Shape of input '{tgt}', (), doesn't match shape " f"{val.shape}.", ident=(src, tgt)
                        )
                        continue
                elif np.squeeze(val).shape != np.squeeze(value).shape:
                    src = self._conn_global_abs_in2out[tgt]
                    self._collect_error(
                        f"Shape of input '{tgt}', {value.shape}, doesn't match " f"shape {val.shape}.", ident=(src, tgt)
                    )
                    continue
        ...
        if src_idx_found:  # auto_ivc connected to local vars with src_indices
            self._collect_error(
                "Attaching src_indices to inputs requires that the shape of the "
    ...
    def _setup_auto_ivcs(self, mode):
        ...
        for src, tgts in auto2tgt.items():
            ...
            if prom not in self._group_inputs:
                self._group_inputs[prom] = [{"use_tgt": tgt, "auto": True, "path": self.pathname, "prom": prom}]
            else:
        ...
        self._var_allprocs_abs2prom[io].update({n: n for n in auto_ivc._var_allprocs_abs2prom[io]})

        self._var_discrete[io].update({"_auto_ivc." + k: v for k, v in auto_ivc._var_discrete[io].items()})
    ...
    def _show_ambiguity_msg(self, prom, metavars, tgts, metadata=None):
        ...
        self._collect_error(
            f"{self.msginfo}: The following inputs, {inputs}, promoted "
